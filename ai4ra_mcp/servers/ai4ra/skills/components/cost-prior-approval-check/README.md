...
Tests the expense against the federal prior-approval requirements of 2 CFR 200.407, using the award's extracted `prior_approval_triggers` as the authoritative list when supplied. This check covers federal sponsor prior approval only — institutional IRB / IACUC / IBC protocol approval is a separate check. When the trigger list is not supplied, it returns `needs_info`.
...
- [`protocol-approval-allowability-check`](../protocol-approval-allowability-check/) — the companion check for institutional IRB / IACUC / IBC approval, a distinct regulatory regime from federal prior approval.
...
Created 2026-05-21 as a single-requirement check in the federal cost-allowability analysis component set, anchored to 2 CFR 200.407.
